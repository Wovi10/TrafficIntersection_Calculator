package utils

object Constants {

    const val DEFAULT_ARM_WIDTH: Double = 3.15
    const val DEFAULT_LANE_WIDTH: Double = 2.75

    const val DEFAULT_ARM_SPEED: Double = 13.889

    const val DEFAULT_STAGE_TIME: Double = 5.0

    const val DEFAULT_INPUT_LANES_NUM: Int = 3
    const val DEFAULT_OUTPUT_LANES_NUM: Int = 1
    const val DEFAULT_ARM_NUM: Int = 4

    const val DEFAULT_DOUBLE: Double = 0.0

    const val EMPTY_STRING: String = ""
    const val NEWLINE: String = "\n"

    const val ZERO: Int = 0
    const val ONE: Int = 1
    const val TWO: Int = 2
    const val FOUR: Int = 4
}